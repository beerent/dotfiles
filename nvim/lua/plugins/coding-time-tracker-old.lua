...
local function load_data()
  ...
  if not success or not data then
    vim.notify("Failed to parse coding time data: " .. (data or "unknown error") .. ". Creating backup and starting fresh.", vim.log.levels.WARN)
    -- Create backup of corrupted file
    local backup_path = file_path .. ".backup." .. os.date("%Y%m%d_%H%M%S")
    vim.fn.rename(file_path, backup_path)
    vim.notify("Corrupted data backed up to: " .. backup_path, vim.log.levels.INFO)
    return {
      daily = {},
      weekly = {},
      monthly = {},
  ...
  -- Ensure session_data is an object, not an array
  if not data.session_data or type(data.session_data) ~= "table" or #data.session_data > 0 then
    data.session_data = {}
  end
...
local function save_data(data)
  ...
  if not file then
    vim.notify("Failed to create temp file for coding time data", vim.log.levels.ERROR)
    return false
  end

  local success, encoded = pcall(vim.json.encode, data)
  if not success then
    vim.notify("Failed to encode coding time data: " .. encoded, vim.log.levels.ERROR)
    file:close()
  ...
  local rename_success = vim.fn.rename(temp_path, file_path)
...
local function save_session_data(is_exit)
  ...
  if new_time_to_save > 0 and new_time_to_save < 86400 then  -- Max 24 hours per save
  ...
  elseif new_time_to_save > 86400 then
    vim.notify("Abnormally large time delta detected (" .. format_time(new_time_to_save) .. "). Skipping save to prevent corruption.", vim.log.levels.WARN)
  end
  ...
  local save_success = save_data(data)
  if not save_success then
    vim.notify("Failed to save coding time data. Your progress may be lost.", vim.log.levels.ERROR)
  end
end
...
-- Save data periodically (every 5 minutes) with error handling
vim.fn.timer_start(300000, function()
  local success, err = pcall(save_session_data, false)
  if not success then
    vim.notify("Error saving coding time data: " .. err, vim.log.levels.ERROR)
  end
end, { ["repeat"] = -1 })

-- Save data on exit with error handling
vim.api.nvim_create_autocmd("VimLeavePre", {
  group = group,
  callback = function()
    local success, err = pcall(save_session_data, true)
    if not success then
      vim.notify("Error saving coding time data on exit: " .. err, vim.log.levels.ERROR)
    end
  end
})
...
if data.session_data and type(data.session_data) == "table" and data.session_data.session_start_timestamp then
  ...
  if time_since_last_session < 86400 then  -- 24 hours in seconds
    ...
    -- DON'T reset session_start_time - keep it as current time for new activity
